...
Если нечетных, то вернуть произведение всех цифр числа. Если нечетных и четных чисел одинаково, то из цифр числа необходимо сделать самое большое число, испольязую 
каждую цифру один раз
*/
...
function maxiNumberFromDigits(a) {
    let resultmaxiNumberFromDigits = a.sort().reverse().join('');
    return resultmaxiNumberFromDigits;
}
...
{   
    ...
    if (sumEven > sumOdd) {
        ...
        return "В числе " + result + " четных и нечетных цифр одинаково, максимальное число их цифр числа это " + maxiNumberFromDigits(n);
    }
}
